...
void settoku5() {
	...
	if (serekutobotanncount2 == 0) {
		...
		if (Pad[PAD_7] == 1 && motonomasunimodoru == 0 && zannzou == 0 && zannzouA == 0 && zannzouB == 0 && zannzouC == 0) {
			...
			playerkonntororulock = 1;//主人公のスライド移動が出来ないようにする。
		}
		if (settokuSeen == 1) {
...
void settoku8() {
	...
	if (serekutobotanncount2 == 3) {
		...
		if (settokubunnkiframe3 > 0) {
			settokubunnki = 0;
			enemyzannzoulock2 = 1;//敵の画像を消すために1にした。

			//メインプログラムのif (mark == 0 && enemy1HP < 1 && marklock == 0)に敵の画像を徐々に消すためにmarkとmarklockを0とした。
			mark = 0;
			marklock = 0;
