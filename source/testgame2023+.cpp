...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int nCmdShow)
{
	...
	// ムービーファイルをロードします。
	MovieGraphHandle = LoadGraph(".\\resource\\images\\180half_f.gif");
	// ムービーを再生状態にします
	...
	sinnzouGraphHandle = LoadGraph(".\\resource\\images\\original.gif");
	// 敵がダメージを受けた時の心臓のムービーを再生状態にします
	PlayMovieToGraph(sinnzouGraphHandle);

	// 雪ムービーファイルをロードします。
	snowGraphHandle = LoadGraph(".\\resource\\images\\20160103081928.gif");
	// 雪ムービーを再生状態にします
	PlayMovieToGraph(snowGraphHandle);

	// アナログノイズムービーファイルをロードします。
	anarogunoizuGraphHandle = LoadGraph(".\\resource\\images\\MOSHED-2020-6-1-23-33-37.gif");
	...
	// ファイルをロードします。
	kodomogif = LoadGraph(".\\resource\\images\\kodomo.gif");
	...
	dennwaHandle = LoadGraph(".\\resource\\images\\dennwasenntaku.gif");

	//会話のファイルをロードします。
	enemytalk1Handle = LoadGraph(".\\resource\\images\\私のビデオ1.gif");
	//会話のファイルをロードします。
	enemytalk2Handle = LoadGraph(".\\resource\\images\\enemytalk2.gif");

	dennwa = LoadGraph(".\\resource\\images\\21b667f1-88cb-4e41-9bdc-41351a55655b.gif");

	// ファイルをロードします。
	SHOTHandle = LoadGraph(".\\resource\\images\\ENEMYGIF1.gif");
	PlayMovieToGraph(SHOTHandle);


	// ヴィオラをロードします。
	vioraHandle = LoadGraph(".\\resource\\images\\MOSHED-2020-6-2-19-37-21.gif");
	PlayMovieToGraph(vioraHandle);

	// 説得中の背景をロードします。
	settokugamenntyuunohaikeiHandle = LoadGraph(".\\resource\\images\\settokugamenntyuunohaikei.gif");
	PlayMovieToGraph(settokugamenntyuunohaikeiHandle);

	// 選択し1をロードします。
	settoku1Handle = LoadGraph(".\\resource\\images\\settoku1.gif");

	// 戦闘後をロードします。
	sentougoHandle = LoadGraph(".\\resource\\images\\senntougonoOUT.gif");

	// コンティニュー画面の背景をロードします。
	countimuHandle = LoadGraph(".\\resource\\images\\MOSHED-2020-6-1-23-49-29.gif");
	PlayMovieToGraph(countimuHandle);

	//戦闘中の背景
	stage1haikeiHandle = LoadGraph(".\\resource\\images\\stage1.gif");
	PlayMovieToGraph(stage1haikeiHandle);

	//戦闘中の背景
	stage2haikeiHandle = LoadGraph(".\\resource\\images\\stage2haikei.gif");
	PlayMovieToGraph(stage2haikeiHandle);

	//女教師の電話映像
	dennwa = LoadGraph(".\\resource\\images\\dennwa2.gif");
	PlayMovieToGraph(dennwa);

	//タイトルの背景。
	taitolhaikeiHandle = LoadGraph(".\\resource\\images\\taitoruhaikei.gif");
	...
	//敵の説得に成功した場合。
		...
		if (Seen == 301) {
			...
			DrawFormatString(1, 680, GetColor(255, 255, 77),
				"★★settokubunnkiframe3は%d,migimukiは%d,kougekidekinailockは%d",
				settokubunnkiframe3, migimuki, kougekidekinailock);

			DrawFormatString(1, 720, GetColor(255, 255, 0),
				"playerkonntororulockは%d,lockは%d,nanameidouX2は%d,nanameidouX3は%d, nanameidouX4は%d,nanameidouX5は%d", 
				playerkonntororulock,lock, nanameidouX2, nanameidouX3, nanameidouX4, nanameidouX5);

			DrawFormatString(1, 740, GetColor(255, 25, 0),
				"playerkonntororulock3は%d,settokubunnkiframe2は%d,rockonは%d,lockonMoveは%d,InputXは%d,InputYは%d", 
				playerkonntororulock3, settokubunnkiframe2, rockon, lockonMove,InputX,InputY);
			// 右トリガーの情報を描画
			DrawFormatString(0, 0, GetColor(255, 255, 255), "LEFTmarkは%d,markは%d,marklockは%d", LEFTmark,mark,marklock);
