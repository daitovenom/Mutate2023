...
#include "jyuujisuraido.h"
#include "syuzinkoukougekigazou.h"
#include "syuzinnkouKENJYUU.h"
#include "settoku2.h"
#include "aruku.h"
#include "enemydamegegazou.h"
#include "orderofcharacters.h"
#include <iostream>
#include <string>
#include <vector>
#include <algorithm>
...
//されるのでここに引っ掛からない様にstayhereの値を1にした。
	...
	if (a == 3) {
		...
		if (enemyKENJYUUcount > 0 && ++enemyKENJYUUcount < 60) {
			if (enemyKENJYUUcount > 0 && enemyKENJYUUcount < 30) {
				...
				DrawBox(0,250,1230,300, RED, TRUE);//四角を描画
	...
	//真ん中//else ifだと他の銃攻撃がまだ動いてるときに他の銃攻撃を受けない時があるので。普通のifにした。
		...
		if (enemyKENJYUUcount > 0 && ++enemyKENJYUUcount < 60) {
			if (enemyKENJYUUcount > 0 && enemyKENJYUUcount < 30) {
				DrawRotaGraph(1260, 350, 5, 0, enemy1KENJYUUHandle[0], TRUE);//元々は350ではなく370であった。
				StartJoypadVibration(DX_INPUT_PAD1, 1000, 3 * enemyKENJYUUcount);//両方のモーターが振動。

				DrawBox(0, 300, 1260, 350, RED, TRUE);//四角を描画
			}
			if (enemyKENJYUUcount > 30 && enemyKENJYUUcount < 60) {
	...
	//一番下。
		...
		if (enemyKENJYUUcount > 0 && ++enemyKENJYUUcount < 60) {
			if (enemyKENJYUUcount > 0 && enemyKENJYUUcount < 30) {
				...
				DrawBox(0, 350, 1290, 400, RED, TRUE);//四角を描画
			}
			if (enemyKENJYUUcount > 30 && enemyKENJYUUcount < 60) {
...
//が攻撃を食らうようにした。
	...
	if (kaitenn2 == 1) {
		++kaitennframe2;//右への回避は分母があると最初がのろくなり攻撃を食らってしまうので、ここは左回避とは違うようにする。
		//ステージ上ではマス移動しかないので、避ける場合はマス移動での避け方にする。
		//if (kaitennframe > 0 && kaitennframe < 2 && motonomasunimodoru == 0 && playerX < 2) {
		//	playerX = playerX + 1;//スライド無しでずっと移動でのyokeruXだと値が一方的に溜まるのでスライド移動なしでの避けるはpos[playerY][playerX][0]にする。
		//}
...
//pos[playerY][playerX][1] + nanameidouY + nanameidouY2 + nanameidouY3が交わり、その範囲がbairituX = 4.6となる条件となる。
	...
	if (DAMEGE == 0) {
		...
		//orderofcharacters.hに画像を書きこんだのでここの部分はいらない。
		/////////////////////////////////////////////////////////
...
//stgae2Player_Update.hにこいつを書いたらラグで攻撃判定が少しずれるのですぐ下にダメージを受けた時の画像が出るここに書いた
	...
	//スライド移動中にRTを押した際にstgae2Player_Update.hの2327行目のif (R1lock == 0)を通るが、
	//RBを押した後はrockonは1のままである。
	//rockonが1のままだと、このプログラムのif (rockon == 0)を通らないため、
	//上に書いてある636行目の「攻撃判定その一(ロックしてから銃による攻撃判定)」のif (hannilock == 2)の{}のif文に含まれる
	//lockmarkXzahyouとlockmarkYzahyouが使えないためkougekiKANOUの値は0のままなので
	//ロックマークが描画されなくてもロックマークで攻撃できない。
	//
	if (rockon == 0) {
